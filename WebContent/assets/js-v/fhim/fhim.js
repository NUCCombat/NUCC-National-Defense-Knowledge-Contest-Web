...
layui.use(['layim','readyMenu'], function(layim){
	...
	var f = setInterval(function() {
		if("" != wimadress){
			...
			if (window.WebSocket) {
				...
				websocket = new WebSocket(encodeURI('wss://'+wimadress)); //wimadress参数在 assets/js-v/sys/index.js 中定义并赋值
			...
			$.ajax({
				...
				type: "POST",
				url: httpurl+'iminterface/getImsign',					//获取皮肤
				...
				success: function(data){
					  ...
					  layim.config({
					    init: {
					      url: httpurl+'iminterface/getList'			//初始化接口
					      ,data: {}
					    }
					    ,members: {
					      url: httpurl+'iminterface/getMembers'			//查看群员接口
					      ,data: {}
					    }
					    ,uploadImage: {
					      url: httpurl+'iminterface/updateImg'			//上传图片接口
					      ,type: 'post' 								//默认post
					    } 
					    
					    ,uploadFile: {
					      url: httpurl+'iminterface/updateFile'			//上传文件接口
					      ,type: 'post' 								//默认post
					    }
					    ...
					    ,msgbox: '../../fhim/msgbox.html' 				//消息盒子页面地址，若不开启，剔除该项即可
					    ,find: '../../fhim/friends/friends_add.html' 	//发现页面地址，若不开启，剔除该项即可
					    ,chatLog: '../../fhim/hismsg/hismsg_list.html' 	//聊天记录页面地址，若不开启，剔除该项即可
  ...
  layim.on('online', function(data){			//监听在线状态的切换事件
	...
	$.ajax({
		...
		type: "POST",
		url: httpurl+'iminterface/editState',
  ...
  layim.on('ready', function(res){
	...
	$.ajax({
		...
		type: "POST",
		url: httpurl+'iminterface/getMsgCount',			//未读消息(加好友群等)总数
    	data: {tm:new Date().getTime()},
		dataType:'json',
		success: function(data){
			if('01' == data.result){
	...
	$.ajax({
		...
		type: "POST",
		url: httpurl+'iminterface/getNoreadMsg',		//未读消息(离线消息)接口
    	data: {tm:new Date().getTime()},
		dataType:'json',
		success: function(data){
			if('has' == data.result){
