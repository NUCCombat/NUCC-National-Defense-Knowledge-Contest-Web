	...
	methods: {
    	...
    	save: function (){
            ...
            $.ajax({
					...
					type: "POST",
					url: httpurl+'judgementquestion/'+this.msg,
			    	data: {JUDGEMENTQUESTION_ID:this.JUDGEMENTQUESTION_ID,
				    TITLE:this.pd.TITLE,
    	...
    	getData: function(){
            ...
            $.ajax({
				...
				type: "POST",
				url: httpurl+'judgementquestion/goEdit',
		    	data: {JUDGEMENTQUESTION_ID:this.JUDGEMENTQUESTION_ID,tm:new Date().getTime()},
				dataType:"json",
				success: function(data){
                     if("success" == data.result){
                     	vm.pd = data.pd;							//参数map
						...
						vm.ANSWER = data.pd.ANSWER == 'true'?true:false;
                     }else if ("exception" == data.result){
                     	showException("判断题",data.exception);	//显示异常
                     	$("#showform").show();
                 		$("#jiazai").hide();
    	...
    	showImg: function(path,TPID){
    		if('' == path || null == path)return false;
			 $("#"+TPID).html('<img width="200" src="'+path+'">');
			 $("#"+TPID).show();
		},
