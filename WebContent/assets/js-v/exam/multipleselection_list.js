var vm = new Vue({
	...
	methods: {
    	...
    	goAdd: function (){
    		 ...
    		 diag.Drag=true;
    		 diag.Title ="新增多项选择题";
    		 diag.URL = '../../exam/multipleselection/multipleselection_edit.html';
    		 diag.Width = 900;
    		 diag.Height = 523;
		...
		getExcel: function (){
			 ...
			 diag.URL = '../../exam/upload_excel.html?type=multipleselection';
			 diag.Width = 600;
			 diag.Height = 130;
			 diag.CancelEvent = function(){ //关闭事件
				 var varSon = diag.innerFrame.contentWindow.document.getElementById('showform');
    	...
    	goEdit: function(id){
    		 ...
    		 diag.Drag=true;
    		 diag.Title ="编辑多项选择题";
    		 diag.URL = '../../exam/multipleselection/multipleselection_edit.html?FID='+id;
    		 diag.Width = 900;
    		 diag.Height = 523;
    		 diag.Modal = true;				//有无遮罩窗口
    		 diag. ShowMaxButton = true;	//最大化按钮
    		...
    		swal({
                ...
                if (willDelete) {
                	...
                	$.ajax({
            			...
            			type: "POST",
            			url: httpurl+'multipleselection/delete',
            	    	data: {MULTIPLESELECTION_ID:id,tm:new Date().getTime()},
            			dataType:'json',
            			success: function(data){
            				 if("success" == data.result){
    				...
    				if(str==''){
    					...
    					if(msg == '确定要删除选中的数据吗?'){
    						...
    						$.ajax({
    							...
    							type: "POST",
    							url: httpurl+'multipleselection/deleteAll?tm='+new Date().getTime(),
    					    	data: {DATA_IDS:str},
    							dataType:'json',
    							success: function(data){
    								if("success" == data.result){
    	...
    	getDic: function(){
      	...
      	//判断按钮权限，用于是否显示按钮
        hasButton: function(){
        	var keys = 'multipleselection:add,multipleselection:del,multipleselection:edit,fromExcel';
        	$.ajax({
        		xhrFields: {
                    withCredentials: true
                },
        		...
        		success: function(data){
        		 if("success" == data.result){
        			vm.add = data.multipleselectionfhadminadd;		//新增权限
        			vm.del = data.multipleselectionfhadmindel;		//删除权限
        			vm.edit = data.multipleselectionfhadminedit;	//修改权限
