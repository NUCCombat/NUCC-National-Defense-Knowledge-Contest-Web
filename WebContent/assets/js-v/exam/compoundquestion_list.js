	...
	methods: {
        ...
        getList: function(){
        	...
        	$.ajax({
        		...
        		type: "POST",
        		url: httpurl+'compoundquestion/list?showCount='+this.showCount+'&currentPage='+this.currentPage,
    	...
    	goAdd: function (){
    		 ...
    		 diag.Drag=true;
    		 diag.Title ="新增复合题";
    		 diag.URL = '../../exam/compoundquestion/compoundquestion_edit.html';
    		 diag.Width = 1000;
    		 diag.Height = 800;
    	...
    	goEdit: function(id){
    		 ...
    		 diag.Drag=true;
    		 diag.Title ="编辑复合题";
    		 diag.URL = '../../exam/compoundquestion/compoundquestion_edit.html?FID='+id;
    		 diag.Width = 1000;
    		 diag.Height = 800;
    		 diag.Modal = true;				//有无遮罩窗口
    		 diag. ShowMaxButton = false;	//最大化按钮
    		...
    		swal({
                ...
                if (willDelete) {
                	...
                	$.ajax({
            			...
            			type: "POST",
            			url: httpurl+'compoundquestion/delete',
            	    	data: {COMPOUNDQUESTION_ID:id,tm:new Date().getTime()},
            			dataType:'json',
            			success: function(data){
            				 if("success" == data.result){
    				...
    				if(str==''){
    					...
    					if(msg == '确定要删除选中的数据吗?'){
    						...
    						$.ajax({
    							...
    							type: "POST",
    							url: httpurl+'compoundquestion/deleteAll?tm='+new Date().getTime(),
    					    	data: {DATA_IDS:str},
    							dataType:'json',
    							success: function(data){
    								if("success" == data.result){
    	...
    	getDic: function(){
      	...
      	//判断按钮权限，用于是否显示按钮
        hasButton: function(){
        	var keys = 'compoundquestion:add,compoundquestion:del,compoundquestion:edit';
        	$.ajax({
        		xhrFields: {
                    withCredentials: true
                },
        		...
        		success: function(data){
        		 if("success" == data.result){
        			vm.add = data.compoundquestionfhadminadd;		//新增权限
        			vm.del = data.compoundquestionfhadmindel;		//删除权限
        			vm.edit = data.compoundquestionfhadminedit;	//修改权限
        		 }else if ("exception" == data.result){
