...
var vm = new Vue({
	...
	methods: {
    	...
    	save: function (){
			...
			return false;
			}else if((this.pd.FOPTION).indexOf((this.ANSWER).toUpperCase()) == -1){
				$("#ANSWER").tips({
					side:3,
		            msg:'答案无效,没有此选项',
		            bg:'#AE81FF',
            ...
            $.ajax({
					...
					type: "POST",
					url: httpurl+'singleelection/'+this.msg,
			    	data: {SINGLEELECTION_ID:this.SINGLEELECTION_ID,
				    TITLE:this.pd.TITLE,
					...
					success: function(data){
                        if("success" == data.result){
                        	...
                        	showException("单选题",data.exception);//显示异常
                        	$("#showform").show();
                    		$("#jiazai").hide();
                        }
                    }
    	...
    	getData: function(){
            ...
            $.ajax({
				...
				type: "POST",
				url: httpurl+'singleelection/goEdit',
		    	data: {SINGLEELECTION_ID:this.SINGLEELECTION_ID,tm:new Date().getTime()},
				dataType:"json",
				success: function(data){
                     if("success" == data.result){
                     	vm.pd = data.pd;							//参数map
    	...
    	appendC: function (value){
    		$("#fields").append('<tr><td>&nbsp;<div class="radio radio-primary d-inline"><input onclick="selection(\''+arLetter[index]+'\')" type="radio" name="radio-p-1" id="f'+index+'"><label for="f'+index+'" class="cr">'+value+'</label></div></td></tr>');
    		index++;
    	},
    	...
    	appendCFromEdit: function (value,ANSWER){
    		if(arLetter[index] == ANSWER){
    			$("#fields").append('<tr><td>&nbsp;<div class="radio radio-primary d-inline"><input onclick="selection(\''+arLetter[index]+'\')" type="radio" name="radio-p-1" id="f'+index+'" checked><label for="f'+index+'" class="cr"></label></div>'+value+'</td></tr>');
    		}else{
    			$("#fields").append('<tr><td>&nbsp;<div class="radio radio-primary d-inline"><input onclick="selection(\''+arLetter[index]+'\')" type="radio" name="radio-p-1" id="f'+index+'"><label for="f'+index+'" class="cr"></label></div>'+value+'</td></tr>');
    		}
    		index++;
    	},
